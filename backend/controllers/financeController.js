...
const API_KEY = '02ca97971a4847ea5b17d1e5';
const BASE_URL = `https://v6.exchangerate-api.com/v6/${API_KEY}/latest/INR`; // base currency INR

async function getLiveRates() {
  try {
    const response = await axios.get(BASE_URL);
    return response.data.conversion_rates; // returns an object like { USD: 83.5, AED: 22.7, ... }
  } catch (err) {
...
const getFinanceSummary = async (req, res) => {
  try {
    ...
    const conversionRates = await getLiveRates();

    // Convert all amounts to INR first
    const totalIncomeINR = incomes.reduce((acc, income) => acc + income.amount * conversionRates[income.currency], 0);
    const totalExpenseINR = expenses.reduce((acc, expense) => acc + expense.amount * conversionRates[expense.currency], 0);
    const balanceINR = totalIncomeINR - totalExpenseINR;
...
const downloadFinanceExcel = async (req, res) => {
  try {
    ...
    // ✅ Excel generation (same as your previous code, just use conversionRates)
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Finance Report');
    ...
    // Add incomes
    incomes.forEach(i => {
      const amountINR = i.amount * conversionRates[i.currency];
      worksheet.addRow({
        type: 'Income',
        title: i.title,
        category: i.category || '-',
    ...
    // Add expenses
    expenses.forEach(e => {
      const amountINR = e.amount * conversionRates[e.currency];
      worksheet.addRow({
        type: 'Expense',
        title: e.title,
        category: e.category || '-',
    ...
    // You can reuse your styling and summary logic here, just ensure you reference conversionRates
